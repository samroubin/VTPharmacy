...
# Join the total population dataset with the age dataset, create a column for % elderly, and modify the GEOID to include the state, county, and town FIPS codes so they can be joined to other population data in later analysis. Clean and reorder data. 
total_population_data <- pop20%>%
  inner_join(elderly_bytown, by = "GEOID")%>%
  rename(total_pop = value,
         fips_subdivision = GEOID,
         subdivision_name = NAME)%>%
  mutate( pct_elderly = ((elderly_pop / total_pop)*100))%>%
  separate(fips_subdivision, into = "fips_state", sep = 2, remove = FALSE)%>%
  separate(fips_subdivision, into = "fips_county", sep = 5, remove = FALSE)%>%
